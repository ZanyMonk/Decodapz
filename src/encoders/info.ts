...
export class Info extends Encoder {
  label = 'Information'
  large = true
  readonly = true
  class = 'no-wrap'
  ...
  async encode(string: string): Promise<string> {
    ...
    report += 'MD5\t: ' + CryptoJS.MD5(string) + '\n'
    report += 'SHA-1\t: ' + await this.digestMessage(string, 'SHA-1') + '\n'
    report += 'SHA-224\t: ' + CryptoJS.SHA224(string) + '\n'
    ...
    report += 'SHA-3\t: ' + CryptoJS.SHA3(string) + '\n'
